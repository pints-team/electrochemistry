...
template <class C> struct pickle_suite : public py::pickle_suite {
...
struct pickle_suite<std::vector<T>> : public py::pickle_suite {
  ...
  static void setstate(py::object obj, py::tuple state) {
    ...
    py::stl_input_iterator<typename std::vector<T>::value_type> begin(state[0]),
        end;
...
BOOST_PYTHON_MODULE(pints_cpp) {
  ...
  py::def("e_implicit_exponential_mesh", e_implicit_exponential_mesh,
          (py::arg("params"), py::arg("Itot"), py::arg("t")),
          "Solves E problem using implicit thomas method, exponentially "
          "expanding mesh");
  py::def("seq_electron_transfer3_explicit", seq_electron_transfer3_explicit,
          (py::arg("params"), py::arg("Itot"), py::arg("t")), "placeholder");
}
